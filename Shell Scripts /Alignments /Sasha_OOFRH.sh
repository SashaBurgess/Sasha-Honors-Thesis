...
files=$workingDir"Raw_Reads_For_Alignment/"$expcond"*.fq.gz"
...
do
	...
	zcat $workingDir"Raw_Reads_For_Alignment/"$sample".fq.gz" | echo $((`wc -l`/4))

	gzip -d $workingDir"Raw_Reads_For_Alignment/"$sample".fq.gz"

	cd $starDir"/first_pass"

	module load star/2.7.3a

	STAR --genomeDir $workingDir"BDGP6.32" --readFilesIn $workingDir"Raw_Reads_For_Alignment/"$sample".fq" --runThreadN 20
	...
	STAR --genomeDir $starDir"/second_index" --readFilesIn  $workingDir"Raw_Reads_For_Alignment/"$sample".fq" --runThreadN 20 --outSAMtype BAM SortedByCoordinate --outFileNamePrefix $workingDir"Sasha_Output_Files/BAM/"$sample
	...
	gzip $workingDir"Raw_Reads_For_Alignment/"$sample".fq"
